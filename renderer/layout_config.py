...
def _build_circle12notes(widget_def: dict) -> list[Mobject]:
  ...
  def map_note_intervals(widget_type: str) -> str:
    if widget_type == "circle_chromatic": return 1
    elif widget_type == "circle_fifths": return 7
  ...
  def map_label_text(widget_type: str) -> str:
    if widget_type == "circle_chromatic": return "Chromatic circle"
    elif widget_type == "circle_fifths": return "Circle of Fifths"
  label_def: dict = widget_def.get("label", {"dummy_ignored": "make dict truthy so it's set as below"})
  if label_def: # is truthy
    ...
    text_chromatic = Text(
        ...
        ).move_to(circle_chromatic).shift(_compute_shift(label_def))
