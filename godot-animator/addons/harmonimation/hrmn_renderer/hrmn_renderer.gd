...
# TODO: UX: "puppet" the auto-generated Animation to tuck all of its tracks separate from the user's Animation.
#   - one AutogenAnimationPlayer holding the autogenerated hrmn animation
#   - one UserAnimationPlayer with a UserAnimation including one property animation track HarmonimationRenderer.time
#   - HarmonimationRenderer.time has a setter which calls seek() on the AutogenAnimationPlayer
...
func apply_animations() -> void:
    ...
    var animation_length := 200.0
    autogen_animation = Utils.setup_animation(animation_player, animation_length)
...
func apply_anims(anims: Dictionary[Variant, Dictionary]) -> void:
    print("harmonimation: running apply_animations() on behalf of %s" % [anims.keys()])
    ...
    Utils.apply_animation(anims, animation_player, autogen_animation)
...
func animate_widget(animation: Animation, widget: HarmonimationWidget) -> void:
    ...
    var anims := widget.hrmn_animate(music_data._music_data_dict)
    Utils.apply_animation(anims, animation_player, animation)
